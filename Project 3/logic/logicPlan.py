...
def sentence1() -> Expr:
    ...
    B = logic.Expr("B")
    C = logic.Expr("C")
    P1 = logic.disjoin(A, B)
    P2 = ~A % logic.disjoin(~B, C)
...
def sentence2() -> Expr:
    ...
    C = logic.Expr("C")
    D = logic.Expr("D")
    P1 = C % logic.disjoin(B, D)
    P2 = A >> logic.conjoin(~B, ~D)
    P3 = ~logic.conjoin(B, ~C) >> A
    P4 = ~D >> C
...
def sentence3() -> Expr:
    ...
    """
    PacmanAlive_1 = logic.PropSymbolExpr("PacmanAlive", 1)
    PacmanAlive_0 = logic.PropSymbolExpr("PacmanAlive", 0)
    PacmanBorn_0 = logic.PropSymbolExpr("PacmanBorn", 0)
    PacmanKilled_0 = logic.PropSymbolExpr("PacmanKilled", 0)
    P1 = PacmanAlive_1 % logic.disjoin(
        logic.conjoin(PacmanAlive_0, ~PacmanKilled_0),
        logic.conjoin(~PacmanAlive_0, PacmanBorn_0),
    )
    P2 = ~logic.conjoin(PacmanAlive_0, PacmanBorn_0)
    P3 = PacmanBorn_0
    return logic.conjoin(P1, P2, P3)
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    # Theorem of non-satisfiability: f models y iff f & ~y
    if findModel(logic.conjoin(premise, ~conclusion)) == False:
        return True
    return False
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for x, y in non_outer_wall_coords:
            # Can we prove whether Pacman is at (x, y)? Can we prove
            # whether Pacman is not at (x, y)?
            C1 = PropSymbolExpr(pacman_str, x, y, time=t)
            if entails(conjoin(KB), C1):
                possible_locations.append((x, y))
                KB.append(PropSymbolExpr(pacman_str, x, y, time=t))
            else:
                C2 = ~C1
                if entails(conjoin(KB), C2):
                    KB.append(~PropSymbolExpr(pacman_str, x, y, time=t))
                else:
                    possible_locations.append((x, y))
...
def mapping(problem, agent) -> Generator:
    ...
    KB.append(
        PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0)
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            # Can we prove whether a wall is at (x, y)? Can we prove
            # whether a wall is not at (x, y)?
            C1 = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))
            C2 = entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y))
            if C1 and not C2:
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            # Can we prove whether Pacman is at (x, y)? Can we prove
            # whether Pacman is not at (x, y)?
            C1 = entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time=t))
            C2 = entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x, y, time=t))
            if C1 and not C2:
                possible_locations.append((x, y))
                KB.append(PropSymbolExpr(pacman_str, x, y, time=t))
            elif not C1 and C2:
