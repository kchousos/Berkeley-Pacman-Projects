...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        # A state is represented by the coordinates of pacman and
        # a tuple of the remaining corners to be visited. Since this
        # is the StartState, all corners are unvisited.
    ...
    def isGoalState(self, state: Any):
        ...
        # If there are no corners left to visit, we have reached
        # goal state.
        return not state[1]
    ...
    def getSuccessors(self, state: Any):
        ...
        # Moved it outside the for loop since it remains
        ...
        for action in [
            ...
            if not hitsWall:
                # cast to list so it can be manipulated
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    while cornersLeft:
        ...
        for corner in cornersLeft:
            distancesOfCorners.append(
                (util.manhattanDistance(coordinates, corner), corner)
            )
        currentDistance, currentCorner = min(distancesOfCorners)
        result += currentDistance
        coordinates = currentCorner
        cornersLeft.remove(currentCorner)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    # checks to see if the distance has already been
    # computed. If so, returns it from heuristicInfo.
    # If not, computes it with mazeDistance and saves it.
    ...
    # Finds closest food to pacman and furthest second food from food
    distancesToFoodFromPos = []
    distancesToFoodFromFood = [0]
    for food in foodLeft:
        distancesToFoodFromPos.append(heuristicInfoDistance(position, food))
        for toFood in foodLeft:
            distancesToFoodFromFood.append(heuristicInfoDistance(food, toFood))

    if distancesToFoodFromPos:
        return min(distancesToFoodFromPos) + max(distancesToFoodFromFood)

    return max(distancesToFoodFromFood)
